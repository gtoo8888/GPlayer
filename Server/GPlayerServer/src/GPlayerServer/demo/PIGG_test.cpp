...
// char *strpbrk(const char *str1, const char *str2) //检索字符串 str1 中第一个匹配字符串 str2
// 中字符的字符，不包含空结束字符 int strcmp(const char *str1, const char *str2) //把 str1 所指向的字符串和 str2
// 所指向的字符串进行比较。 int strcasecmp (const char *s1, const char *s2); //
// C语言中判断字符串是否相等的函数，忽略大小写。s1和s2中的所有字母字符在比较之前都转换为小写 int strncasecmp(const char
// *s1, const char *s2, size_t n);   // 比较参数s1 和s2 字符串前n个字符，比较时会自动忽略大小写的差异。 size_t
// strspn(const char *str1, const char *str2) // 检索字符串 str1 中第一个不在字符串 str2 中出现的字符下标。 size_t
// strlen(const char *str) // 计算字符串 str 的长度，直到空结束字符，但不包括空结束字符。 char *strrchr(const char *str,
// int c) // 在参数 str 所指向的字符串中搜索最后一次出现字符 c（一个无符号字符）的位置。
