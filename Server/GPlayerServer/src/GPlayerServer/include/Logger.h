...
private:
    ...
    void *async_write_log() {                      // 异步写日志,这是一个回调函数
        ...
        while (PIGG_log_queue->pop(single_log)) {  // 从阻塞队列中取出一个日志string，写入文件
            ...
            fputs(single_log.c_str(),
                  PIGG_fp);  // 把字符串写到指定的流中，也就是把前面的字符串写到后面的FILE*这个类型的变量里面
            // PIGG_mutex.unlock();
        }
    }
...
#define LOG_DEBUG(format, ...)                                         \
    if (true) {                                                        \
        PIGG_log::get_instance()->write_log(0, format, ##__VA_ARGS__); \
...
#define LOG_WARN(format, ...)                                          \
    if (true) {                                                        \
        PIGG_log::get_instance()->write_log(2, format, ##__VA_ARGS__); \
        PIGG_log::get_instance()->flush();                             \
    }
#define LOG_ERROR(format, ...)                                         \
    if (true) {                                                        \
        PIGG_log::get_instance()->write_log(3, format, ##__VA_ARGS__); \
        PIGG_log::get_instance()->flush();                             \
    }
