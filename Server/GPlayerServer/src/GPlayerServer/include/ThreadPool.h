...
PIGG_threadpool<T>::PIGG_threadpool(int actor_model, PIGG_connection_pool *coonPool, int thread_number, int max_request)
    : PIGG_actor_model(actor_model),
      PIGG_thread_number(thread_number),
      PIGG_max_requests(max_request),
      PIGG_threads(NULL),
      PIGG_connPool(coonPool) {
    if (thread_number <= 0 || max_request <= 0) throw std::exception();  // 这个是为了下一句不要报错
    PIGG_threads = new pthread_t[PIGG_thread_number];                    // 线程id初始化
    if (!PIGG_threads) throw std::exception();
