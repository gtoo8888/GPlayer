...
PIGG_http_conn::PIGG_HTTP_CODE PIGG_http_conn::parse_request_line(char *text) {
    ...
    *PIGG_url++ = '\0';
    ...
    if (!PIGG_version) return BAD_REQUEST;
    *PIGG_version++ = '\0';
    PIGG_version += strspn(PIGG_version, " \t");
    if (strcasecmp(PIGG_version, "HTTP/1.1") != 0) return BAD_REQUEST;
    ...
    if (!PIGG_url || PIGG_url[0] != '/') return BAD_REQUEST;
...
PIGG_http_conn::PIGG_HTTP_CODE PIGG_http_conn::do_request() {
    ...
    if (PIGG_cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        int i = 0;
        for (i = 5; PIGG_string[i] != '&'; ++i) name[i - 5] = PIGG_string[i];
        name[i - 5] = '\0';
    ...
    if (!(PIGG_file_stat.st_mode & S_IROTH)) return FORBIDDEN_REQUEST;

    if (S_ISDIR(PIGG_file_stat.st_mode)) return BAD_REQUEST;
