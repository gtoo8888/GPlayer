...
// Json::Value是 JsonCpp 中最基本、最重要的类，用于表示各种类型的对象，JsonCpp 支持的对象类型可见 Json::ValueType
// 枚举值。 Json::Reader是用于读取的，说的确切点，是用于将字符串转换为 Json::Value 对象的。
// Json::Writer类是一个纯虚类，并不能直接使用。在此我们使用 Json::Writer
// 的子类：Json::FastWriter、Json::StyledWrite、Json::StyledStreamWriter。 （2）Jsoncpp 中所有对象、类名都在 namespace
// json 中，包含 json.h 即可
